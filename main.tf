...
resource "null_resource" "ecr_image" {
  ...
  provisioner "local-exec" {
    command = "AWS_PROFILE=${var.aws_profile} AWS_DEFAULT_REGION=${var.aws_region} AWS_ACCOUNT=${data.aws_caller_identity.current.account_id} bash ${path.module}/bin/build.sh ${var.dockerfile_file_path} ${var.ecr_repository_url}:${var.docker_image_tag}"
  }
}